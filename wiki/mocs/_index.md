...
- **[[icenterlib|ICenterLib top-level]]** — the 723-file shared library (35 folders): Common, AppSettings, Connections + all the per-system wrappers (ISAH, iCenter, SmtProduction, CAD, PCFNet, …). `#safety-relevant`. **Top-MOC + 3 root files written 2026-06-18.**
  - **[[icenterlib-isah|ICenterLib/ISAH]]** — 65 files wrapping ISAH ERP. Identity, workflow docs, lookups, dossier hierarchy, production hierarchy, Part + dispatch, TimeRegistration + collector, Icenter2Isah sync, 20+ leaves, plus modern DataServices/Handlers/ViewModels/Helpers. `#safety-relevant`. **Sub-MOC + 11 module + 11 business-rule notes written 2026-06-18. Coverage 63/63 source files (100%, 2 generated).**
  - **[[icenterlib-icenter|ICenterLib/iCenter]]** — 29 files wrapping the iCenter DB: ProductionMachines, IPorder→IPbatch→IPpacket→IPpart hierarchy, FrmIdentification + WebClockAssistant, leaves (Part/XmlFile/Servicedesk/TimeRegistration/CoatingLayerThickness). `#safety-relevant`. **Sub-MOC + 4 module + 5 business-rule notes written 2026-06-18. Coverage 24/24 source files (100%, 5 generated).**
...
- **[[trutopslib|TruTopsLib]]** — Trumpf TruTops file-format parser (`.GEO` + TopsFile + PMI), maintained in **mirrored VB + C#**. `#safety-relevant`. **MOC + 1 business-rule note (DXF color-7 hazard) written 2026-06-18. Coverage 57/57 source files (100%, 8 generated/config).**
