...
- **[[icenterlib|ICenterLib top-level]]** — the 723-file shared library (35 folders): Common, AppSettings, Connections + all the per-system wrappers (ISAH, iCenter, SmtProduction, CAD, PCFNet, …). `#safety-relevant`. **Top-MOC + 3 root files written 2026-06-18.**
  - **[[icenterlib-isah|ICenterLib/ISAH]]** — 65 files wrapping ISAH ERP. Identity (Employee/User/Customer/Vendor/Company), workflow docs (ShopDoc/PurDoc), lookups (Selection/Setting/MachGrp), dossier hierarchy (DossierMain/Detail/Extra/DocFolder), production hierarchy (ProductionHeader/PBOO/PBOM/PBOS + BillOfOper/BillOfMat templates). `#safety-relevant`. **Sub-MOC + 6 module + 4 business-rule notes written 2026-06-18.**
